...
export class TurnosService {
  ...
  private apiUrl = `${environment.apiUrl}/master-data/turnos`;

  getAll(filters: TurnoFilters = {}): Observable<PaginatedTurnos> {
    let params = new HttpParams();
    if (filters.search) params = params.set('search', filters.search);
    if (filters.active !== undefined) params = params.set('active', String(filters.active));
    if (filters.page)  params = params.set('page',  String(filters.page));
    if (filters.limit) params = params.set('limit', String(filters.limit));
    return this.http.get<PaginatedTurnos>(this.apiUrl, { params });
  }
  ...
  create(dto: Partial<Turno>): Observable<Turno> {
    return this.http.post<Turno>(this.apiUrl, dto);
  }

  update(id: string, dto: Partial<Turno>): Observable<Turno> {
    return this.http.put<Turno>(`${this.apiUrl}/${id}`, dto);
  }

  toggleActive(id: string, active: boolean): Observable<Turno> {
    return this.http.patch<Turno>(`${this.apiUrl}/${id}/active`, { active });
  }
